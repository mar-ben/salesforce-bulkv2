...
class BulkApi:
    ...
    def __init__(
        ...
        self.access_token = ""
        self.instance_url = (
            "https://test.salesforce.com" if sandbox else "https://login.salesforce.com"
        )
        auth_url = self.instance_url + "/services/oauth2/token?grant_type="
        auth_url = (
            auth_url
            + "{}&client_id={}&client_secret={}&username={}&password={}".format(
                self.GRANT_TYPE, client_id, client_secret, username, password
            )
        )
    ...
    def create_query_job(self, soql):
        """create a bulk job"""
        uri = self.instance_url + "/services/data/{}/jobs/query".format(
            self.api_version
        )
        ...
        response = requests.post(
            url=uri, headers=self.__get_request_header(), json=body
        )
        print(response.json())
    ...
    def get_query_job_status(self, job_id):
        ...
        response = requests.get(url=uri, headers=self.__get_request_header())
    ...
    def get_query_results(self, job_id, max_records=None):
        """ return the query resuls"""
        sf_locator = ""
        result_data = ""
        request_uri = (
            self.instance_url
            + "/services/data/{}/jobs/query/{}/results".format(self.api_version, job_id)
        )
        uri = ""
        if max_records:
    ...
    def create_job(self, object_api_name, operation, external_id_field_name=None):
        ...
        request_url = self.instance_url + "/services/data/{}/jobs/ingest".format(
            self.api_version
        )
        ...
        response = requests.post(
            headers=self.__get_request_header(), url=request_url, json=body
        )
        print(response.json())
    ...
    def upload_data(self, job, data):
        uri = self.instance_url + "/services/data/{}/jobs/ingest/{}/batches/".format(
    ...
    def upload_complete(self, job):
        uri = self.instance_url + "/services/data/{}/jobs/ingest/{}/".format(
            self.api_version, job
        )
        body = {"state": "UploadComplete"}

        response = requests.patch(
            headers=self.__get_request_header(), url=uri, json=body
        )
        return response
    ...
    def get_job_status(self, job):
        ...
        response = requests.get(headers=self.__get_request_header(), url=uri)
        return response.json()
    ...
    def upsert(self, object_api_name, external_id_field_name, data):
        ...
        job_id = self.create_job(object_api_name, "upsert", external_id_field_name)
        self.upload_data(job_id, data)
        self.upload_complete(job_id)
        self.wait_for_job_complete(job_id)
        return job_id
