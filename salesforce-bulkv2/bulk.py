...
class Bulk:
    ...
    def create_query_job(self,soql):
        uri=self.instance_url+"/services/data/{}/jobs/query".format(self.api_version)
        ...
        response=requests.post(url=uri,headers=self.get_request_headers(),json=body);
        print('job id :{}'.format(response.json()['id']))
        return response.json()['id']
    

    def get_query_job_status(self,job_id):
        uri=self.instance_url+"/services/data/{}/jobs/query/{}".format(self.api_version,job_id)
    ...
    def get_query_results(self,job_id,max_records=None):
        sf_locator=''
        result_data=''
        request_uri=self.instance_url+"/services/data/{}/jobs/query/{}/results".format(self.api_version,job_id)
        uri=''
        if max_records:
    ...
    def create_job(self,object_api_name,operation,external_id_field_name=None):
        ...
        response=requests.post(headers=self.get_request_headers(),url=request_url,json=body)
    ...
    def upload_data(self,job,data):
        ...
        response=requests.put(headers=self.get_request_headers(),url=uri,data=data)
        return response
    ...
    def upload_complete(self,job):
        ...
        response=requests.patch(headers=self.get_request_headers(),url=uri,json=body)
        return response;
    ...
    def upsert(self,object_api_name,external_id_field_name,data):
        ...
        job_id=self.create_job(object_api_name,'insert',external_id_field_name);
        self.upload_data(job_id,data)
        self.upload_complete(job_id)
        self.wait_for_job_complete(job_id)
    ...
    def get_rejected_records(self,job_id):
        uri=self.instance_url+'/services/data/{}/jobs/ingest/{}/failedResults'.format(self.api_version,job_id)
    
        response=requests.get(headers=self.get_request_headers(),url=uri)
        return response.content.decode()
    
    def get_success_records(self,job_id):
        uri=self.instance_url+'/services/data/{}/jobs/ingest/{}/successfulResults'.format(self.api_version,job_id)
